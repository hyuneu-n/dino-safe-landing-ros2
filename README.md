...
> **2026-09-23 마일스톤:** 환경 제작을 마무리하고, 교수 피드백에 따라 전방 Depth 비용 지도와
> 1·2·3스텝 지역 경로 생성기를 구현했다. 현재 DINO는 착륙 판단에 사용하며, 순항 DINO 결합은 비교 실험 전이다.
...
현재 구현된 것은 RGB-D 착륙점 판단, 기본 미션 상태머신, 전방 Depth 기반 다중 스텝 지역 경로 생성과 시각화다.
...
**현재 검증 범위는 환경·센서·배치다.** 새 맵의 전체 비행·착륙 성공률은 아직 측정하지 않았다.
캠퍼스·교량·산동네의 고도 12/34/36/96m 목적지 4곳은 환경·센서 검증용이며, 자동 비행에는 기존 제어기의
고도 처리 수정이 필요하다. 차량은 현재 정적이며, 반복 배송·사용자 장애물 배치·통합 대시보드는 다음 단계다.
...
대시보드와 반복 배송은 아직 미구현이다. 기술 측면의 다음 우선순위는 지역 계획 반복 실험과 순항 DINO 결합 비교다.
...
전달한다. 2026-09-23 현재 UI에서 활성화한 종단 간 검증 코스는 **물류창고 → 맥도날드** 한 곳이다.
