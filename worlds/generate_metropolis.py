...
class City:
    ...
    def spot(self, ident, label, kind, x,y, note, z=0):
        self.spots.append(dict(id=ident,label=label,kind=kind,x=x,y=y,z=z,note=note,
                               requires_elevation_support=z>0,flight_validated=False))
...
def generate(out,seed=0):
    ...
    c.models.append(DRONE_TEMPLATE.replace("<far>500</far>","<far>2500</far>").format(sx=c.origin['x'],sy=c.origin['y'],sz=7))
    ...
    xml+='<gui><camera name="user_camera"><pose>-360 -560 360 0 0.42 0.6</pose></camera></gui>'
    # Terrain and riverbank meshes are generated by metropolis_region.
    xml+=''.join(c.models)+'</world></sdf>'
    ET.fromstring(xml)
    c.out.write_text(xml)
    manifest=dict(layout='metropolis',seed=seed,start_xy=[c.origin['x'],c.origin['y']],target_xy=[150,0],
                  origin=c.origin,landmarks=c.landmarks,waypoints_xy=c.departure_waypoints,
                  ...
                  districts=[dict(id='downtown',bounds=[-180,180,-125,125]),dict(id='industrial',bounds=[180,376,-125,125]),
                             ...
                             dict(id='old_town',bounds=[-850,-210,-100,250]),
                             dict(id='mountains',bounds=[-750,1400,200,950]),
                             dict(id='river',bounds=[-950,1550,-425,-125]),
                             dict(id='south_bank',bounds=[-900,1500,-850,-350])])
